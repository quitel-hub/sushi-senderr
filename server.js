...
//           },
//         });
//         es.json({ token, owner: { id: owner.id, username: owner.username } });
//       } catch (ownerError) {
//         console.error("Ошибка при создании/обновлении владельца для неудачной попытки:", ownerError);
//       }
...
//     // ----------------------------------------------------------------
//     // --- ИСПРАВЛЕНИЕ 401: Здесь был неверный токен ---
//     // ----------------------------------------------------------------
//     // Было: if (ownerToken !== "41cf7b310f781b709f7f8d2b4e8c2b08") {
//     if (ownerToken !== "sushi.master.admin.2024@secure-icon.com") {
//       return res.status(401).json({ message: "Неверный токен." });
//     }
...
// app.post("/admin/broadcast", authenticateSession, async (req, res) => {
//   try {
//     // 1. Валидация данных
//     const { userIds, subject, htmlContent } = emailBroadcastSchema.parse(req.body);

//     // 2. Получаем email'ы выбранных пользователей из Prisma
//     // (Ваша модель называется 'Customer', судя по коду /api/customers)
//     const customers = await prisma.customer.findMany({
//       where: {
//         id: { in: userIds },
...
//     // Рекомендую добавить SENDGRID_FROM_EMAIL в ваш .env файл
//     const msg = {
//       to: emails, // SendGrid автоматически обработает массив
//       from: process.env.SENDGRID_FROM_EMAIL || 'your-verified-email@yourdomain.com', // ВАШ EMAIL
//       subject: subject,
