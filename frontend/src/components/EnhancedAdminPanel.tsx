...
// // УДАЛЕНО: import AdminLogin from './AdminLogin';
...
// // ИСПРАВЛЕНИЕ: Компонент теперь принимает пропсы
...
//   // УДАЛЕНО: Внутреннее состояние аутентификации
...
//   // === ИЗМЕНЕНИЕ: УДАЛЕНЫ НЕНУЖНЫЕ (ДУБЛИРУЮЩИЕ) СОСТОЯНИЯ ===
...
//   // УДАЛЕНО: Проверка аутентификации при загрузке (теперь это в App.tsx)
...
//   // ИСПРАВЛЕНИЕ: `fetchData` теперь использует `adminToken` и вызывает `onLogout` при ошибке 401
//   const fetchData = useCallback(async () => {
//     // Если токена нет (например, при выходе), не делаем запросы
//     if (!adminToken) {
//       setIsLoading(false);
//       return;
//     }
...
//     try {
//       // ИСПРАВЛЕНИЕ: Используем `adminToken` из пропсов
//       const headers = {
...
//       // ИСПРАВЛЕНИЕ: Проверяем на 401 (сессия истекла / невалидный токен)
...
//         onLogout(); // Вызываем logout, переданный из App.tsx
//         return;
//       }
...
//       setIsLoading(false);
//     }
//   }, [t, adminToken, onLogout]); // Добавляем adminToken и onLogout в зависимости

//   // Автоматическая синхронизация (без изменений, но теперь зависит от `fetchData`)
//   useEffect(() => {
//     if (!autoSync) return;
...
//   }, [autoSync, fetchData]); // Добавляем fetchData в зависимости

//   // Первичная загрузка данных (без изменений)
//   useEffect(() => {
//     fetchData();
//   }, [fetchData]);

//   // === ИЗМЕНЕНИЕ: ЛОГИКА `handleBroadcast` ОБНОВЛЕНА ===
...
//         url = '/api/admin/broadcast'; // <--- НОВЫЙ ENDPOINT
//         body = JSON.stringify({
//           userIds: selectedRecipients, // <--- НОВЫЙ PAYLOAD
//           subject: broadcastMessage.title,
//           htmlContent: broadcastMessage.body
//         });
//         // -----------------------------------------------------------
//       } else {
//         // --- Оставляем СТАРУЮ логику для SMS и WhatsApp ---
//         const apiEndpoints = {
//           sms: '/api/owner/broadcast/sms',
//           whatsapp: '/api/owner/broadcast/whatsapp'
//         };
//         // Убедимся, что канал правильный
...
//           'x-owner-token': adminToken // Используем токен из пропсов
//         },
//         body: body
//       });
...
//   // ИСПРАВЛЕНИЕ: `handleLogout` теперь вызывает `onLogout` из пропсов
//   const handleLogout = () => {
//     onLogout();
//   };

//   // --- (Остальная часть компонента: formatDate, getStatusBadge, фильтры, статистика, JSX) ---
//   // --- (Они не требуют изменений, т.к. используют уже обновленное состояние) ---
...
//   };
// // Функция для массовой рассылки SMS
//   const handleSendMassSms = async (e) => {
//     e.preventDefault();
//     if (!smsMessage) {
//       alert(t('admin.broadcast.sms.errorNoMessage')); // 'Введите текст сообщения'
//       return;
//     }
//     if (!window.confirm(t('admin.broadcast.sms.confirm', { count: totalSubscribed }))) { // 'Вы уверены, что хотите отправить SMS X подписчикам?'
//       return;
//     }
...
//           "x-owner-token": adminToken,
//         },
//         body: JSON.stringify({ body: smsMessage }),
//       });

//       const data = await response.json();

//       if (response.ok) {
//         setBroadcastStatus({ type: 'success', message: `${t('admin.broadcast.sms.success')}: ${JSON.stringify(data.summary)}` }); // 'Рассылка завершена'
//         setSmsMessage("");
//       } else {
//         setBroadcastStatus({ type: 'error', message: data.message || t('admin.broadcast.error') }); // 'Ошибка рассылки'
//       }
//     } catch (error) {
...
//     }
//   };
//   // УДАЛЕНО: Рендеринг <AdminLogin />
//   // if (!isAuthenticated) { ... }

//   // Рендеринг JSX (остается без изменений, т.к. он уже защищен в App.tsx)
//   return (
//     <div className="enhanced-admin-panel">
//       {/* ... (весь ваш JSX для хедера, статистики, фильтров и вкладок) ... */}
...
//       {/* ... (весь JSX для контента вкладок остается таким же) ... */}
...
//                 onClick={handleBroadcast} // <--- ВЫЗЫВАЕТ ОБНОВЛЕННУЮ ФУНКЦИЮ
