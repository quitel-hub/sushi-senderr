...
//     try {
//       // ПОЛУЧИТЬ НАСТОЯЩИЙ ТОКЕН ИЗ ХРАНИЛИЩА
//       const token = localStorage.getItem('adminAuthToken'); // (Название может быть другим)

//       // Если токена нет, нет смысла делать запрос
//       if (!token) {
//         setIsAuthenticated(false); // Выйти из системы, если токена нет
//         return;
//       }
      
//       const headers = {
//         // ИСПОЛЬЗОВАТЬ НАСТОЯЩИЙ ТОКЕН
//         // Формат заголовка может быть другим, например 'Authorization': `Bearer ${token}`
//         'x-owner-token': token 
//       };
...
//   // ИСПРАВЛЕНИЕ: `handleBroadcast` теперь использует `adminToken`
//   const handleBroadcast = async () => {
//     if (!broadcastMessage.title.trim() || !broadcastMessage.body.trim()) return;
//     if (selectedRecipients.length === 0) return;
//     if (!adminToken) {
...
//     setIsBroadcasting(true);
//     try {
//       // ИСПРАВЛЕНИЕ: Убран жестко прописанный email
//       const apiEndpoints = {
//         sms: '/api/owner/broadcast/sms',
//         email: '/api/owner/broadcast/email',
//         whatsapp: '/api/owner/broadcast/whatsapp'
